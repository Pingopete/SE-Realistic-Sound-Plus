    ...
    internal static class AudioDebugOverlay
    {
        ...
        public static void Draw()
        {
            ...
            try
            {
                ...
                DrawLine(2, "type  eng  amb  spd  count  volume  cue  | route tr atm sc base fin d p", HeaderColor, 0.50f, centerX, startY, rowHeight);
                ...
                for (int i = 0; i < shown; i++)
                {
                    ...
                    string diagnostic = AudioDiagnostics.TryGetCueSnapshot(row.CueName, out AudioDiagnostics.CueSnapshot snapshot)
                        ...
                        : (row.EngineCandidate || row.SpeedAmbientCandidate ? " UNCONTROLLED" : string.Empty);
                    string text = string.Format(
                        CultureInfo.InvariantCulture,
                        "{0}    {1}    {2}    {3}   x{4,2}   {5:0.00}   {6}{7}",
                        row.Kind,
                        row.EngineCandidate ? "*" : "-",
