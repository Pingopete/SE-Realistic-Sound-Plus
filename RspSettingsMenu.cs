...
namespace RealisticSoundPlus
{
    internal sealed class RspSettingsMenu : MyGuiScreenBase
    {
        ...
        private void BuildScrollableContent(MyGuiControlParent content)
        {
            ...
            AddSlider(content, ref y, "Reposition Air Bias", "blockrepoairbias", 0.1f, 10f, 1, () => SettingsManager.Current.PlayerFilterBlockRepositionAirBias, "Pulls the emitter toward the AIR-PATH portal (doorway/stairwell) vs the direct through-structure line. 1 = physical loudness blend; raise it to localise harder to the opening. NOTE: this is the position blend, not the Open Path Bias above (which only picks the route).");
            AddSlider(content, ref y, "Reposition Smoothing", "blockreposeslew", 1f, 2000f, 0, () => SettingsManager.Current.PlayerFilterBlockRepositionSlewMs, "Temporal smoothing (ms) of the emitter's position as its blended target moves. Higher = smoother, slower glide between positions, less rubberbanding. Snaps once on first placement, no slide out from the block.");
            AddSlider(content, ref y, "Air Path Reach", "blockairpathreach", 1f, 16f, 0, () => SettingsManager.Current.PlayerFilterBlockAirPathReach, "How far (grid cells) the around-corner flood-fill searches beyond the direct line. Raise this to find longer detours - multi-flight/switchback staircases need more reach. Higher = more CPU per blocked source.");
            AddToggle(content, ref y, "Air Path Through Open Blocks", () => SettingsManager.Current.PlayerFilterBlockAirPathThroughBlocks, value => SettingsManager.Current.PlayerFilterBlockAirPathThroughBlocks = value, "Sound goes where air goes: let the detour pass through any non-airtight block (stairs, catwalks, gratings, railings), not just empty cells + open doors. Fixes stairwells packed with stair blocks. Sealed walls/closed doors still block.");
            AddSlider(content, ref y, "Open Path Bias", "blockairopenbias", 0f, 30f, 0, () => SettingsManager.Current.PlayerFilterBlockAirPathOpenBias, "How strongly the air path prefers OPEN air over grated/non-sealing blocks. Raise this so the path routes up the open stairwell instead of taking a short hop straight up through a grated floor. 0 = pure shortest path.");
