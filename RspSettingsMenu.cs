...
namespace RealisticSoundPlus
{
    internal sealed class RspSettingsMenu : MyGuiScreenBase
    {
        ...
        private void BuildScrollableContent(MyGuiControlParent content)
        {
            ...
            AddSlider(content, ref y, "Portal Averaging", "blockportalavg", 1f, 2000f, 0, () => SettingsManager.Current.PlayerFilterBlockRepositionPortalAvgMs, "Stage 1 smoothing: averages the raw portal as it jumps between whole grid cells (ms). Higher = a steadier mid-point between recent doorway positions, less grid-snapping.");
            AddSlider(content, ref y, "Reposition Glide", "blockreposeslew", 1f, 2000f, 0, () => SettingsManager.Current.PlayerFilterBlockRepositionSlewMs, "Stage 2 smoothing: how gradually the emitter eases toward the averaged target (ms). Higher = slower, smoother slide. No hard snaps either direction now.");
