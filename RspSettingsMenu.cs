...
namespace RealisticSoundPlus
{
    internal sealed class RspSettingsMenu : MyGuiScreenBase
    {
        ...
        private void BuildScrollableContent(MyGuiControlParent content)
        {
            ...
            AddSlider(content, ref y, "Reposition Smoothing", "blockreposeslew", 1f, 2000f, 0, () => SettingsManager.Current.PlayerFilterBlockRepositionSlewMs, "Temporal smoothing (ms) of the emitter's position as its blended target moves. Higher = smoother, slower glide between positions. Snaps once on first placement, no slide out from the block.");
