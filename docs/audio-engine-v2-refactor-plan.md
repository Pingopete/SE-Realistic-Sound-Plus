...
Live branch: `v2/live-audio-engine`

Reference snapshot branch: `reference/current-build-before-v2`
...
The current experimental build was preserved as `reference/current-build-before-v2` before implementation started. That branch contains the active experiment work, the user sound organization workbook, and this plan at the point of the V2 branch split.

The live implementation branch is `v2/live-audio-engine`. It starts from `main`, then brings forward this plan and the workbook so V2 can be built cleanly instead of inheriting every experiment.
...
- `DirectionalSpoolAudioPatch`: six-direction grouped spool emitter experiment.
- `CenteredSpoolSuppressionPatch`: centered grid cue suppression experiment.
